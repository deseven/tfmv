...
if [ -f "$pb/compilers/pbcompiler" ]; then
	...
	"$pb/compilers/pbcompiler" -u -e "$loc/${name}.app" "$loc/../mapedit.pb" > /dev/null || die "failed to build ${name}"
	if [ -d "$loc/$name.app" ]; then
		...
		zip -r9 "$shortName-osx.zip" "${name}.app" > /dev/null || die "failed to pack $shortName"
